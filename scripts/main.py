...
class GetCompetitсors:
  ...
  def main(self) -> str:
    course_data: list[dict] = self._get_course_data(self.course_url) # for example
    lowest_position: Union[None, int] = self._get_lowest_position(self.search_option, course_data=course_data)
    if not lowest_position: return
...
class General:
  def __init__(self, snils) -> None:
    ...
    self.TECH_COURSES_DATA2: dict[str, str] = {
      'ee0043c0-88ce-4a0e-b3be-d46605fd8fd1': {'name': 'Прикладная математика и информатика',
                                              'amount': 77},
      'c9484eea-449e-42aa-bda4-3111b180b1c0': {'name': 'Информатика и вычислительная техника (Компьютерное моделирование и проектирование)',
                                              'amount': 43},
  ...
  def _get_course_data(self, course_url: str) -> list[dict]:
    ...
    if not course_data: print(course_url, course_data, course_base_data)
  ...
  def main(self):
    ...
    for url in self.coursers_urls:
      try:
        ...
        napr = self.TECH_COURSES_DATA[url.split("/")[-1]]
        if dd:
          for q in dd:
            if (q.value in self.competitors_data) and (self.competitors_data[q.value]['course_id'] != napr):
              dd.pop(dd.index(q))
          dd = [self.competitors_data[q.value] for q in dd]
          napr2 = self.TECH_COURSES_DATA2[url.split("/")[-1]]
          balls = f'\nБаллы выше: '
          if len(dd) >= 5:
            balls += f'{dd[-1]["points"]} {dd[-2]["points"]} {dd[-3]["points"]} {dd[-4]["points"]} {dd[-5]["points"]}'
          else:
